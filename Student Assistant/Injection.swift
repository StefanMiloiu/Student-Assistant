...
final class Injection {
    ...
    */
    private func buildContainer() -> Container {
        ...
        container.register(AssignmentRepo.self) { resolver in
            AssignmentRepo(dataManager: resolver.resolve(DataManagerProtocol.self)!)
        }
        // Registering the ExamRepo, which also depends on the DataManagerProtocol
        container.register(ExamRepo.self) { resolver in
            ExamRepo(dataManager: resolver.resolve(DataManagerProtocol.self)!)
        }
        // Registering the TrackTimeRepo, which depends on the DataManagerProtocol
        container.register(TrackTimeRepo.self) { resolver in
            TrackTimeRepo(dataManager: resolver.resolve(DataManagerProtocol.self)!)
        }
        // Registering the AssignmentListViewModel, which depends on the AssignmentRepo
        container.register(AssignmentListViewModel.self) { resolver in
            AssignmentListViewModel(assignmentRepo: resolver.resolve(AssignmentRepo.self)!)
        }
